...
namespace ModelForm.Exporter
{
    public class Exporter
    {
        ...
        public static Exporter Instance(string titles,string dataGetter, string fileType, string compressType)
        {
            ...
            if (titles != null)
            {
                var title = JsonConvert.DeserializeObject<List<List<Column>>>(titles);
                export.Title(title);
            }
            if (dataGetter != null)
                export.Data(dataGetter);

            if (fileType != null)
                export.Export(fileType);

            if (compressType != null)
                export.Compress(compressType);
