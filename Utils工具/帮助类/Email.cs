...
namespace Utils工具
{
    public  class Email
    {
        public string  SendMail()
        {
            ...
            objMailMessage.From = new MailAddress(fromAddress, "钱富通", System.Text.Encoding.UTF8);//发送方地址
            objMailMessage.Subject = "测试邮件";       //邮件标题
            objMailMessage.Body = "这是一个测试邮件";  //邮件内容
            objMailMessage.BodyEncoding = Encoding.UTF8;//邮件编码
            objMailMessage.SubjectEncoding = Encoding.UTF8;
            objMailMessage.IsBodyHtml = true;          //邮件正文是否为html格式             
            objMailMessage.Priority = MailPriority.Normal; //设置优先级
            ...
            objSmtpClient.Credentials = new System.Net.NetworkCredential(fromAddress, code);//发送方的邮件地址，密码
